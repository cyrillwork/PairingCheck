...
struct COM com;
COMMTIMEOUTS timeouts = {}; //Initializing COMMTIMEOUTS structure
...
int writeSerial(int fd, char* buffer, int count) {
  ...
  ret = WriteFile(com.hComm, static_cast<LPCVOID>(buffer), count, (LPDWORD)(&rc), NULL);
...
int openSerial(char* portname, int opt) {
  ...
  {
    com.port = static_cast<LPCWSTR>(calloc(1, sizeof(char) * strlen("\\\\.\\COM10") + 1));
    strncat((char*)com.port, "\\\\.\\", strlen("\\\\.\\"));
  }
  //COMx
  else
  {
    com.port = static_cast<LPCWSTR>(calloc(1, sizeof(char) * 5));
  }

  strncat((char*)com.port, portname, 4);
